...
const createLibsCss = gulp.series(createCss,function (cd) {
	gulp.src('app/css/libs.css') // Выбираем файл для минификации
		.pipe(cssnano()) // Сжимаем
		.pipe(rename({ suffix: '.min' })) // Добавляем суффикс .min
		.pipe(gulp.dest('app/css/')); // Выгружаем в папку app/css
	cd();
...
const watch = gulp.series(createCss, createLibsCss, createBrowserSync, function (cd) {
...
gulp.task('build', gulp.series(cleanDist, createCss, createLibsCss, function (cd) {
